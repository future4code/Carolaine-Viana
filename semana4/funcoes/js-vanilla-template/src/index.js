...
//a) 10, 50
//B) Nao apareceria nada console do navegador
...
//Resposta = está pegando todos os numeros pares desse array e multiplicando por eles mesmo.
...
const resultado = somaNumeros(2, 2)
...
const resposta = teste(2, 2)

console.log(resposta)

//C

function mensagem(aux){
    for(let i=0; i<=9; i++){
        console.log(aux)
    }
}

mensagem("Meu nome nao é Johnny")
...
//C
let qntd = 0
function quantidadePares(meusarrays){
    for(indice of meusarrays){
        if(indice%2===0){
            qntd++
        }
    }
    console.log(`Quantidade de pares: ${qntd}`)
}
quantidadePares(array)

//D

function manipulaPar(variosarrays){
    for(indice of variosarrays){
        if(indice%2===0){
            console.log(indice, true)
        }
    }
}
manipulaPar(array)
